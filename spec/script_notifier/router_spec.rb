...
describe ScriptNotifier::Router do
  ...
  context "process! method" do
    ...
    it "gets the next message, processes it and sends the results when told to process!" do
      subject.should_receive(:get_next_notification_message).once.and_return(sample_message)
      subject.should_receive(:process_notification).once.with(sample_message).and_return(sample_result)
      subject.should_receive(:send_results).once.with(sample_result)
      subject.send(:process!)
    end

    it "returns nil and halts processing if get_next_notification_message returns nil" do
      subject.stub!(:get_next_notification_message).once.and_return(nil)
      subject.should_not_receive(:process_notification)
      subject.should_not_receive(:send_results)
      subject.send(:process!)
    end
  end
  ...
  context "getting the next message" do
    it "returns a hash of the JSON message" do
      ...
      subject.send(:get_next_notification_message).should == sample_message
    end
    ...
    it "returns nil if there is an error parsing the message" do
      ...
      subject.send(:get_next_notification_message).should == nil
    end
  end
  ...
  context "sending the result message" do
    it "returns a hash of the JSON message" do
      ...
      subject.send(:send_results, sample_result)
    end
  end
